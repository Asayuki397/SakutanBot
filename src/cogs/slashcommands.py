...
class 슬래시커맨드(commands.Cog, description = "슬래시 커맨드와 관련된 명령어입니다."):
    ...
    @commands.command(name = "동기화", description = "[관리자 전용][슬래시 불가] global 함수를 해당 서버에 동기화합니다.")
    ...
    @commands.command(name = "클리어", description = "[관리자 전용][슬래시 불가] 슬래시 커맨드를 해당 서버에서 제거합니다.")
    @commands.has_permissions(administrator=True)
